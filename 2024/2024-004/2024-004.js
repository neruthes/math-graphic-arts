(async function () {
    ...
    const apply_distortion_field = function (x, y) {
        ...
        const text_color_neighbor = text1_image.getPixelColor(xx + 2500, yy + 2500 + 1);
        const text_color_float = (Math.floor(text_color / 256) % 256) / 255; // Using the Blue channel
        const text_color_float_neighbor = (Math.floor(text_color_neighbor / 256) % 256) / 255; // Using the Blue channel
        const color_tangent = text_color_float_neighbor - text_color_float;
        // if (text_color > 300) {
        //     console.log(`text_color: `, text_color);
        //     console.log(`text_color_float: `, text_color_float);
        // }
        let yyy = y + (text_color_float + 0.99 * color_tangent) * 40;
        let xxx = x + (text_color_float + 0.5 * color_tangent) * 40;
        return [xxx, yyy];
    }
    ...
    for (let itr_x = -150; itr_x <= 150; itr_x++) {
        let path_data1 = `M `;
        let path_data2 = `M `;
        for (let itr_y = -3000; itr_y <= 3000; itr_y += 4) {
            let yy = itr_y;
        ...
        // Yellow lower
        SVG_CONTENTS_OUTER += `<path fill="none" opacity="0.9" transform="rotate(0)" stroke="#F3C623" stroke-width="6"  d="${path_data2}" />\n`;
        // Green higher
        SVG_CONTENTS_OVERLAY += `<path fill="none" opacity="0.5" transform="rotate(0)" stroke="#0D7C66" stroke-width="7"  d="${path_data1}" />\n`;
    }
...
-->
<rect fill="#E0E0E0" x="-2500" y="-2500" width="5000" height="5000" />
